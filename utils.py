...
def evaluate_agent(agent, test_states, test_bm_preds, test_y):
    with torch.no_grad():
        weights = agent.select_action(test_states)  # (2816, 9)  # SRAY  (500, 9)  not (2816,9)
    act_counter = Counter(weights.argmax(1))
    act_sorted  = sorted([(k, v) for k, v in act_counter.items()])
    print (weights.shape); #sray (500,9)
    weights = np.expand_dims(weights, -1)  # (2816, 9, 1)   Sray it is (500,9,1)
    print (weights.shape); #sray  (500,9,1)
    print (test_bm_preds); #sray  (6897, 9, 24)
